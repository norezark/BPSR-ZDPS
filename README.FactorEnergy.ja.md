...
現在の配布物は **修正版1（fix1）** です。値が省略された移動状態の通知で同期エラーになる不具合を修正しました。省略された整数値は移植元と同様にゼロとして扱い、位置・リソースの値がない通知でも装着因子情報を失わないようにしています。以前の版を使っている場合はその版を終了し、修正版を起動してからゲームへ再ログインしてください。
